...
function App(){
  ...
  useEffect(()=>{
    if(!debts?.length||!readyToSave)return;
    const totalDebt=debts.reduce((s,d)=>s+Math.max(parseFloat(d.balance)||0,0),0);
    const currentTotalDebt=parseFloat(profile?.totalDebt)||0;
    ...
    setProfile(p=>({...p,totalDebt:Math.round(totalDebt),netWorth:Math.round(totalAssets-totalDebt)}));
  },[debts]);
  useEffect(()=>{
  ...
  const effectiveTotalDebt=liveDebtTotal!==null?Math.round(liveDebtTotal):(activeProfile.totalDebt||0);
  const liveProfile=hasLiveData?{...activeProfile,shareValue:liveShareValue,cryptoValue:liveCryptoValue,totalAssets:liveAssets,totalDebt:effectiveTotalDebt,netWorth:liveAssets-effectiveTotalDebt}:{...activeProfile,totalDebt:effectiveTotalDebt,netWorth:(activeProfile.totalAssets||0)-effectiveTotalDebt};
